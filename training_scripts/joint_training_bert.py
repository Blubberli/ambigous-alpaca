...
def predict(test_loader, model, device):
    ...
    for batch in test_loader:
        batch["device"] = device
        out = model(batch).squeeze().to("cpu")
        phrase_vec = batch["phrase"]
        composed_phrase = F.normalize((out + phrase_vec), p=2, dim=1)
        for i in range(len(out)):
            pred = out[i]
            predictions.append(pred.detach().numpy())
            combined.append(composed_phrase[i].detach().numpy())
        loss = get_loss_cosine_distance(composed_phrase=out, original_phrase=batch["l"])
        test_loss.append(loss.item())
        orig_phrases.append(batch["phrase"])
    predictions = np.array(predictions)
    combined = np.array(combined)
    return predictions, combined, np.average(test_loss)
...
if __name__ == "__main__":
    ...
    prediction_path_dev_combined = str(Path(config["model_path"]).joinpath(save_name + "combined_dev_predictions.npy"))
    prediction_path_test_combined = str(
        Path(config["model_path"]).joinpath(save_name + "combined_test_predictions.npy"))
    rank_path_dev = str(Path(config["model_path"]).joinpath(save_name + "_dev_ranks.txt"))
    rank_path_dev_combined = str(Path(config["model_path"]).joinpath(save_name + "combined_dev_ranks.txt"))
    ...
    if valid_model:
        ...
        valid_predictions, combined_predictions, valid_loss = predict(valid_loader, valid_model, device)
        save_predictions(predictions=valid_predictions, path=prediction_path_dev)
        save_predictions(predictions=combined_predictions, path=prediction_path_dev_combined)
        ...
        ranker_attribute = NearestNeigbourRanker(path_to_predictions=prediction_path_dev_combined,
                                                 embedding_extractor=feature_extractor,
                                                 data_loader=rank_loader,
                                                 all_labels=labels,
                                                 y_label="label", max_rank=1000)
        ...
        logger.info("result for combined representation")
        logger.info(ranker_attribute.result)
        logger.info("quartiles : %s" % str(ranker_attribute.quartiles))
        logger.info(
            "precision at rank 1: %.2f; precision at rank 5 %.2f" % (ranker_attribute._map_1, ranker_attribute._map_5))
        ...
        if config["eval_on_test"]:
            ...
            test_predictions, combined_predictions, test_loss = predict(test_loader, valid_model, device)
            save_predictions(predictions=test_predictions, path=prediction_path_test)
            save_predictions(predictions=combined_predictions, path=prediction_path_test_combined)
            ...
            ranker_attribute = NearestNeigbourRanker(path_to_predictions=prediction_path_test,
                                                     ...
                                                     all_labels=labels,
                                                     y_label="label", max_rank=1000)
            ranker_attribute.save_ranks(rank_path_test)

            logger.info("test result for learned attribute representation")
            logger.info(ranker_attribute.result)
            logger.info("test quartiles : %s" % str(ranker_attribute.quartiles))
            logger.info(
                "precision at rank 1: %.2f; precision at rank 5 %.2f" % (
                    ranker_attribute._map_1, ranker_attribute._map_5))
            logger.info("test accuracy: %.2f; f1 score: %.2f" % (ranker_attribute.accuracy, ranker_attribute.f1))

            logger.info("saved ranks to %s" % rank_path_test)

            ranker_attribute = NearestNeigbourRanker(path_to_predictions=prediction_path_test_combined,
                                                     embedding_extractor=feature_extractor,
                                                     data_loader=rank_loader,
                                                     all_labels=labels,
                                                     y_label="label", max_rank=1000)
            ...
            logger.info("test result for combined representation")
            logger.info(ranker_attribute.result)
            logger.info("quartiles : %s" % str(ranker_attribute.quartiles))
            logger.info(
                "precision at rank 1: %.2f; precision at rank 5 %.2f" % (
